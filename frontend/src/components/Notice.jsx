          ...
          {notice &&
            notice.map((item, index) => {
              return (
                <div
                  key={item._id}
                  className="border-black border-2 w-full rounded-md shadow-sm py-4 px-6 mb-4 relative"
                >
                  {(router.pathname === "/faculty" ||
                    router.pathname === "/admin") && (
                    <div className="absolute flex justify-center items-center right-4 bottom-3">
                      <span className="text-sm bg-black px-4 py-1 text-white rounded-full">
                        {item.type}
                      </span>
                      <span
                        className="text-2xl group-hover:text-black ml-2 cursor-pointer hover:text-red-500"
                        onClick={() => deleteNoticehandler(item._id)}
                      >
                        <MdDeleteOutline />
                      </span>
                      <span
                        className="text-2xl group-hover:text-black ml-2 cursor-pointer hover:text-black"
                  ...
                  >
                    {item.title}
                    {item.link && (
                      <span className="text-2xl group-hover:text-black ml-1">
                        <IoMdLink />
                      </span>
                    )}
                  </p>
      ...
      {open && (
        <form className="mt-8 w-full">
          ...
          <div className="w-[40%] mt-4">
            ...
            <select
              id="type"
              className="px-2 bg-blue-50 py-3 rounded-sm text-base w-[80%] accent-black-700 mt-4"
              value={data.type}
          ...
          {edit && (
            <button
              ...
              className="bg-black text-white mt-6 px-6 rounded text-lg py-2 hover:bg-blue-50 hover:text-black"
            >
              Update Notice
            </button>
          )}
